class StatsController < ApplicationController
  def index
    ...
    links = Stat.where(link_id: params[:id]).all.group_by { | record | record.created_at.year.to_s 
                                                                      + "-" 
                                                                      + record.created_at.month.to_s.rjust(2, '0') 
                                                                      + "-" 
                                                                      + record.created_at.day.to_s.rjust(2, '0') 
                                                                      + " " 
                                                                      + record.created_at.hour.to_s.rjust(2, '0') 
                                                          }
    data = { Stats: [] }
